    ...
    public function convert(int $candidateId, array $input): int
    {
        ...
        try {
            ...
            if ($hackathonId === false) {
                ...
                $stmt = $pdo->prepare('INSERT INTO hackathons (source_id, source_event_id, canonical_url, official_url, title, organizer_name, platform_name, description, hackathon_type, start_at_utc, end_at_utc, registration_deadline_utc, prize_text, location_text, status, verification_status, what_to_know, last_seen_at, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
                $stmt->execute([$sourceId, 'candidate:' . $candidate['id'], $officialUrl, $officialUrl, $title, $input['organizer_name'] ?? null, $input['platform_name'] ?? ($candidate['source_name'] ?? null), $input['description'] ?? null, $input['hackathon_type'] ?? null, $this->dateValue($input['start_at_utc'] ?? null), $end, $this->dateValue($input['registration_deadline_utc'] ?? null), $input['prize_text'] ?? null, $input['location_text'] ?? null, $status, 'unreviewed', $input['what_to_know'] ?? 'Review official rules, eligibility, judging, prize terms, and submission requirements before signing up.', $now, $now, $now]);
